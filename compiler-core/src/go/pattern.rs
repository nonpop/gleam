use expression::{is_type_public_and_transparent, is_type_single_constructor};
use im::HashMap;
use num_bigint::BigInt;
use std::sync::OnceLock;

use super::{expression::is_go_scalar, *};
use crate::{
    analyse::Inferred,
...
pub static ASSIGNMENT_VAR: &str = "_a";

#[derive(Debug)]
enum Index {
    Int(bool, usize), // public, index
    String(EcoString),
    ByteAt(usize),
    IntFromSlice {
        start: usize,
...
impl<'module_ctx, 'expression_gen, 'a> Generator<'module_ctx, 'expression_gen, 'a> {
    ...
    fn push_string(&mut self, s: EcoString) {
        self.path.push(Index::String(s));
    }

    fn push_int(&mut self, public: bool, i: usize) {
        self.path.push(Index::Int(public, i));
    }
    ...
    fn push_string_times(&mut self, s: EcoString, times: usize) {
        for _ in 0..times {
    ...
    fn path_document(&self) -> Document<'a> {
        concat(self.path.iter().map(|segment| {
            match segment {
                Index::Int(public, i) => docvec![
                    ".",
                    to_go_positional_field_name((*i).try_into().expect("u64"), *public)
                ],
                ...
                Index::StringPrefixSlice(i) => docvec!["[", i, ":]"],
            }
        }))
    }
    ...
    fn guard(&mut self, guard: &'a TypedClauseGuard) -> Output<'a> {
        Ok(match guard {
            ClauseGuard::Equals { left, right, .. } if is_go_scalar(left.type_()) => {
            ...
            ClauseGuard::NotEquals { left, right, .. } if is_go_scalar(left.type_()) => {
                let left = self.wrapped_guard(left)?;
                let right = self.wrapped_guard(right)?;
                docvec![
                    to_go_package_name(PRELUDE_MODULE_NAME),
            ...
            ClauseGuard::FieldAccess {
                ...
                let public = is_type_public_and_transparent(
                    &self.expression_generator.module,
                    &container.type_().named_type_name().expect("named type").1,
                );

                docvec![
                    self.guard(container)?,
                    ".",
                    to_go_field_name(label, public),
                    "_f()"
                ]
            }
            ...
            ClauseGuard::ModuleSelect {
                ...
                to_go_package_name(module_alias),
                ".",
                to_go_name(label, true)
            ],
            ...
            ClauseGuard::Constant(constant) => {
                return expression::guard_constant_expression(
                    &self.expression_generator.dep_modules,
                    self.expression_generator.module,
    ...
    /// patterns can be referenced, but in the compiled Go they have not
    /// yet been defined.
        ...
        match pattern {
            ...
            Pattern::Variable { name, type_, .. } => {
                let type_ = type_doc(
                    &self.expression_generator.module,
                    type_,
                    &mut self.expression_generator.tracker,
                    &self.expression_generator.generic_type_ids_in_scope,
                );
                self.push_assignment(subject.clone(), name, type_);
                Ok(())
            }
            ...
            Pattern::List { elements, tail, .. } => {
                ...
                for pattern in elements {
                    self.push_string("Head()".into());
                    self.traverse_pattern(subject, pattern)?;
                    self.pop();
                    self.push_string("Tail()".into());
                }
                self.pop_times(elements.len());
                if let Some(pattern) = tail {
                    self.push_string_times("Tail()".into(), elements.len());
                    self.traverse_pattern(subject, pattern)?;
                    self.pop_times(elements.len());
                }
                Ok(())
            ...
            Pattern::Constructor {
                ...
                panic!("Go generation performed with uninferred pattern constructor");
            }
            ...
            Pattern::StringPrefix {
                ...
                if let AssignName::Variable(right) = right_side_assignment {
                    self.push_string_prefix_slice(utf8_no_escape_len(left_side_string));
                ...
                if let Some((left, _)) = left_side_assignment {
                    ...
                    self.assignments.push(Assignment {
                        ...
                        name: left,
                        var,
                        type_: docvec![to_go_package_name(PRELUDE_MODULE_NAME), ".String_t"],
                    });
                }
                Ok(())
            }
            ...
            Pattern::Constructor {
                constructor: Inferred::Known(PatternConstructor { field_map, .. }),
                arguments,
                name,
                type_,
                ..
            } => {
                let (type_module, type_name) = type_.named_type_name().expect("named type");
                let public = if type_.is_result() || type_.is_nil() {
                    true
                } else {
                    is_type_public_and_transparent(&self.expression_generator.module, &type_name)
                };
                let single_constructor = if type_.is_result() {
                    false
                } else if type_.is_nil() {
                    true
                } else {
                    is_type_single_constructor(
                        &self.expression_generator.dep_modules,
                        &self.expression_generator.module,
                        &type_module,
                        &type_name,
                ...
                if !single_constructor {
                    self.push_variant_check(subject.clone(), name.clone(), public);
                }

                for (index, arg) in arguments.iter().enumerate() {
                    if !single_constructor {
                        if public {
                            self.push_string(eco_format!("As{name}()"));
                        } else {
                            self.push_string(eco_format!("as{name}()"));
                        }
                    }
                    match field_map {
                        None => self.push_int(public, index),
                        Some(FieldMap { fields, .. }) => {
                            ...
                            match label {
                                Some(label) => self.push_string(to_go_name(label, public)),
                                None => self.push_int(public, index),
                            }
                        }
                    }
    ...
    fn push_assignment(&mut self, subject: Document<'a>, name: &'a EcoString, type_: Document<'a>) {
    ...
    fn push_string_prefix_check(&mut self, subject: Document<'a>, prefix: &'a str) {
        self.expression_generator.tracker.go_strings_used = true;
    ...
    fn push_bool_check(&mut self, subject: Document<'a>, expected_to_be_truthy: bool) {
        self.checks.push(Check::Bool {
            expected_to_be_true: expected_to_be_truthy,
            subject,
            path: self.path_document(),
        })
    }
    ...
    fn push_variant_check(&mut self, subject: Document<'a>, kind: EcoString, public: bool) {
    ...
    pub fn into_doc(self, match_desired: bool) -> Document<'a> {
        match self {
            ...
            Check::Variant {
                ...
                let kind = to_go_name(&kind, true);
                if public {
                    if match_desired {
                        docvec![subject, path, ".Is", kind, "()"]
                    } else {
                        docvec!["!", subject, path, ".Is", kind, "()"]
                    }
                } else {
            ...
            Check::Equal { subject, path, to } => {
                let negation = if match_desired { "" } else { "!" };
                docvec![
                    to_go_package_name(PRELUDE_MODULE_NAME),
                    ".Bool_t(",
                    negation,
            ...
            Check::BitArrayLength {
                ...
                let length_check = if has_tail_spread {
                    eco_format!(" >= {expected_bytes}").to_doc()
                } else {
                    eco_format!(" == {expected_bytes}").to_doc()
                };
                if match_desired {
                    docvec!["len(", subject, path, ")", length_check,]
                } else {
                    docvec!["!(len(", subject, path, ")", length_check, ")",]
                }
            }
            Check::StringPrefix {
                ...
                if match_desired {
                    docvec![
                        to_go_package_name(PRELUDE_MODULE_NAME),
                        ".Bool_t(strings.HasPrefix(string(",
                        subject,
                        path,
                        "), ",
                        prefix,
...
/// Calculates the length of str as utf8 without escape characters.
fn utf8_no_escape_len(str: &EcoString) -> usize {
